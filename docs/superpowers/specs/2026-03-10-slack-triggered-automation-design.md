# Slack-Triggered Automation for TGG Ops

## Overview

A local daemon that monitors a dedicated Slack channel (`#tgg-builds`) and automatically triggers Claude Code to implement requested changes, deploy them, and report results back to Slack.

## Flow

```
#tgg-builds message
  → tgg-daemon.sh (launchd, every 60s)
...
### 1. `scripts/tgg-daemon.sh`

Main loop script executed by launchd every 60 seconds.

**Steps per run:**
1. Acquire lock (`/tmp/tgg-daemon.lock`) via `flock` — exit if held
...
**Plist:** `~/Library/LaunchAgents/com.tgg.daemon.plist`
...
- `scripts/tgg-daemon-start.sh` — `launchctl load` the plist
- `scripts/tgg-daemon-stop.sh` — `launchctl unload` the plist
- `scripts/tgg-daemon-status.sh` — check if running, show last processed message
...
```
scripts/
  tgg-daemon.sh          # Main daemon script
  tgg-daemon-start.sh    # Load launchd job
  tgg-daemon-stop.sh     # Unload launchd job
  tgg-daemon-status.sh   # Check status
data/
  last_processed_ts            # Watermark file
  daemon.log                   # Daemon logs
~/.tgg-daemon.env        # SLACK_BOT_TOKEN, SLACK_CHANNEL_ID
~/Library/LaunchAgents/
  com.tgg.daemon.plist   # launchd job definition
```

## Setup Steps

1. Create Slack app at api.slack.com with bot token scopes
2. Create `#tgg-builds` channel, invite the bot
3. Save bot token and channel ID to `~/.tgg-daemon.env`
4. Run `scripts/tgg-daemon-start.sh`
